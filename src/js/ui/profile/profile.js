...
async function loadProfile() {
  ...
  try {
    ...
    // Define posts BEFORE using userPosts.length
    const userPosts = p.posts ?? [];

    const avatar = p.avatar?.url
      ? `<img src="${esc(p.avatar.url)}" alt="${esc(p.name)} avatar" class="profile-avatar">`
      : "";
